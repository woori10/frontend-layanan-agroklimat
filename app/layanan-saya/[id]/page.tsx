...
function getSteps(status: string, hasTagihan: boolean, tagihanLunas: boolean, layananId: number) {
  ...
  const isVerifikasiDone = ["menunggu_pembayaran", "diproses", "selesai_diproses", "menunggu_konfirmasi", "selesai"].includes(status);
  ...
  const isDisetujuiDone = ["diproses", "selesai_diproses", "selesai"].includes(status);
  const isDisetujuiActive = status === "menunggu_konfirmasi";

  const isDiprosesDone = ["selesai_diproses", "selesai"].includes(status);
  const isDiprosesActive = status === "diproses";

  const isSelesaiDone = ["selesai_diproses", "selesai"].includes(status);
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className="rounded-2xl border border-zinc-200/80 bg-white p-6 md:p-8 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-y-6 gap-x-8 text-sm">
                  <div>
                    <span className="block text-xs font-medium text-zinc-600 dark:text-zinc-550 uppercase tracking-wider mb-1">
                      Nama Lengkap
                    </span>
              ...
              <div className="rounded-2xl border border-zinc-200 bg-zinc-50 dark:bg-zinc-900/30 p-6 md:p-8 shadow-sm">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6">
                  ...
                  <Link href={`/layanan-saya/${tiket.id}/ajukan-kembali`}>
                    <button
                      disabled={!["selesai", "selesai_diproses"].includes(tiket.status)}
                      className={`inline-flex items-center justify-center gap-2 rounded-xl px-5 py-3.5 text-sm font-bold transition whitespace-nowrap ${["selesai", "selesai_diproses"].includes(tiket.status)
