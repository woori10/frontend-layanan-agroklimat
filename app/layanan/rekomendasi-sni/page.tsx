...
import RekomendasiSNIStep2Form from "@/components/form/layanan/rekomendasi-sni/page";
import {
  AlertCircle,
  CheckCircle2,
  ArrowLeft,
...
const LAYANAN_ID = 2; // Rekomendasi & Penilaian Kesesuaian Agroklimat/Hidrologi (SNI)

export default function RekomendasiSNIPage() {
  ...
  const handleSubmit = async (step2Data: { jenisData: string; alasan: string }) => {
    ...
    try {
      ...
      if (!response.ok) {
        let errMsg = "Gagal mengajukan rekomendasi SNI.";
        if (data.message) {
      ...
      console.error("[Submit SNI] Error:", err);
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between pb-4 border-b border-zinc-200 dark:border-zinc-800">
          <div>
            ...
            <h1 className="text-2xl sm:text-3xl font-extrabold text-zinc-900 dark:text-white tracking-tight">
              Formulir Rekomendasi SNI
            </h1>
            <p className="text-zinc-500 dark:text-zinc-400 text-xs sm:text-sm mt-1 leading-relaxed">
              Rekomendasi & Penilaian Kesesuaian Agroklimat / Hidrologi berdasarkan Standar Nasional Indonesia.
            </p>
          </div>
        </div>
          ...
          <div className="rounded-xl bg-emerald-50 p-6 text-sm text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-400 border border-emerald-200/40 dark:border-emerald-900/40 flex flex-col items-center text-center gap-3 shadow-lg transition duration-300">
            ...
            <div className="space-y-1">
              ...
              <p className="text-emerald-700 dark:text-emerald-400">
                Formulir rekomendasi SNI telah diajukan dengan nomor tiket <span className="font-extrabold text-emerald-900 dark:text-white">{createdTiketNo}</span>.
              </p>
