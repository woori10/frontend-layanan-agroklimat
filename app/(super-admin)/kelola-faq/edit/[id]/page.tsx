...
export default function EditFaqPage({ params }: PageProps) {
    ...
    useEffect(() => {
        ...
        const loadFaqDetail = async () => {
            try {
                const res = await fetch(`${getApiUrl()}/faq/${faqId}`, {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            console.error(err);
            setError(err.message || "Terjadi kesalahan saat memperbarui FAQ.");
        } finally {
            setSubmitting(false);
        }
    };
                            ...
                            <div className="flex items-center gap-3">
                                <div className="flex-1 space-y-2 pb-4 border-b border-b-zinc-200 dark:border-b-zinc-800">
                                    ...
                                    <p className="text-sm text-[var(--foreground)] dark:text-zinc-500">
                                        Perbarui data pertanyaan, jawaban, urutan tampil, atau status publikasi FAQ ini.
                                    </p>
                                </div>
                            </div>
                        </div>
