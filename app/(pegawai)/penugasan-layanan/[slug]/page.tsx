...
export default function PenugasanLayananPegawaiSlugPage() {
  ...
  useEffect(() => {
    ...
    getLayananBySlug(slug)
      .then((layananData) => {
        ...
        return getUnitTeknisTikets();
      })
      .then((unitTikets: Tiket[]) => {
        const validData = Array.isArray(unitTikets)
          ? unitTikets.filter((t: any) => t.status !== "menunggu_persetujuan_kepala_balai")
          : [];
        setTikets(validData);
      })
  ...
  // Filter tiket khusus layanan ini
  const serviceTikets = tikets.filter((t) => {
    if (layanan) {
      return t.layanan?.id === layanan.id || t.layanan?.slug === slug;
    } else if (!isNaN(Number(slug))) {
      return t.layanan?.id === Number(slug);
    }
    return true;
  });

  const isPeminjamanAlat = slug === "peminjaman-alat" || layanan?.slug === "peminjaman-alat";
  const countSemua = serviceTikets.length;
  const countDiproses = serviceTikets.filter((t) => t.status === "diproses" || t.status === "diterima").length;
  const countDipinjam = serviceTikets.filter((t) => t.status === "dipinjam").length;
  const countMenungguBayar = serviceTikets.filter((t) => t.status === "menunggu_pembayaran").length;
  const countSelesai = serviceTikets.filter((t) => ["menunggu_konfirmasi", "selesai"].includes(t.status)).length;

  useEffect(() => {
    let result = [...serviceTikets];
    ...
    if (selectedStatus !== "semua") {
      if (selectedStatus === "selesai") {
        ...
        result = result.filter((t) => t.status === "diproses" || t.status === "diterima");
      } else if (selectedStatus === "dipinjam") {
        result = result.filter((t) => t.status === "dipinjam");
    ...
    setCurrentPage(1);
  }, [tikets, selectedStatus, searchQuery, sortOrder, layanan, slug]);
  ...
  const goToDetail = (noTiket: string) => {
    router.push(`/penugasan-layanan/${slug}/${noTiket}`);
  };
          ...
          <div className="flex justify-between items-center w-full relative overflow-hidden space-y-3">
            <div className="space-y-3">
              <h1 className="text-2xl font-semibold md:text-3xl text-[var(--foreground)] dark:text-zinc-50">
                Layanan <span className="text-green-color">{layanan?.nama_layanan || "Memuat..."}</span>
              </h1>
              <p className="text-sm font-medium">
                Mengelola tiket layanan masuk {layanan?.nama_layanan || ""} untuk unit teknis Anda
              </p>
            </div>
          </div>
                  ...
                  <div className="inline-flex min-w-full sm:min-w-0 items-center rounded-xl bg-secondary-green-color p-1.5 dark:border-zinc-700 dark:bg-zinc-800 gap-1">
                    {(isPeminjamanAlat
                      ? [
                          ...
                          { label: "Sedang Dipinjam", value: "dipinjam", count: countDipinjam },
            ...
            <div className="overflow-x-auto">
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 text-zinc-400 dark:text-zinc-500">
                  ...
                  <p className="text-xs">Belum ada pengajuan untuk layanan ini di unit Anda.</p>
                </div>
              ) : (
                      ...
                      >
                        ...
                        <td className="px-6 py-5.5 whitespace-nowrap text-xs text-center">
                          <div className="flex justify-center">
                            <StatusLayananBadge status={tiket.status} layananSlug={slug} />
                          </div>
                        </td>
                        <td className="px-6 py-5.5 whitespace-nowrap text-xs text-center">
                          <div className="flex justify-center">
