...
export default function PersetujuanLayananSlugPage() {
  ...
  const fetchTikets = () => {
    ...
    getLayananBySlug(slug)
      ...
      .then((data) => {
        // Kepala Balai hanya menangani tiket yang telah diverifikasi admin
        const relevantTikets = Array.isArray(data)
          ? data.filter((t: any) => !["menunggu_verifikasi", "diajukan"].includes(t.status))
          : [];
        setTikets(relevantTikets);
      })
  ...
  const goToDetail = (noTiket: string) => {
    router.push(`/persetujuan-layanan/${slug}/${noTiket}`);
  };
          ...
          <div className="flex justify-between items-center w-full relative overflow-hidden space-y-3">
            <div className="space-y-3">
              <h1 className="text-2xl font-semibold md:text-3xl text-[var(--foreground)] dark:text-zinc-50">
                Persetujuan Tiket <span className="text-green-color">{layanan?.nama_layanan || "Memuat..."}</span>
              </h1>
              <p className="text-sm font-medium">
                Menyetujui dan mendisposisikan tiket untuk layanan {layanan?.nama_layanan || ""} ke unit teknis terkait
              </p>
            </div>
          </div>
            ...
            <div className="overflow-x-auto">
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-20 text-zinc-400 dark:text-zinc-500">
                  ...
                  <p className="text-xs">Belum ada pengajuan tiket untuk kategori ini.</p>
                </div>
              ) : (
